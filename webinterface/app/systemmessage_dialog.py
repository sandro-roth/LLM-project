...
def render_systemmessage_dialog(active_key: str, get_effective_system_message):
    ...
    # Basisschlüssel + Bump-Zähler + Buffer + aktueller Widget-Key
    base = f"systemmessage_editor__{active_key}"
    rev_key = f"{base}__rev"
    buf_key = f"{base}__buf"
    widget_key_key = f"{base}__widget_key"

    # Initialisierung
    if rev_key not in st.session_state:
        st.session_state[rev_key] = 0
    if buf_key not in st.session_state:
        st.session_state[buf_key] = overrides.get(active_key, "")
    ...
    if c1_clicked:
        override = (overrides.get(active_key) or "").strip()
        effective = override if override else (get_effective_system_message(active_key) or "")
        st.session_state[buf_key] = effective
        st.session_state[rev_key] += 1
        st.toast('Aktueller Systemprompt geladen')
        #st.rerun()

    if c2_clicked:
        # Aktuellen Widget-Key auslesen und Text holen
        current_widget_key = st.session_state.get(widget_key_key)
        text_val = st.session_state.get(current_widget_key, "")
        overrides[active_key] = (text_val or "").strip()
        st.session_state[buf_key] = overrides[active_key]
    ...
    # Textarea jetzt mit **neuem** Key rendern (unten ~~im Code~~, oben im Layout)
    rev = st.session_state[rev_key]
    widget_key = f"{base}__v{rev}"
    st.session_state[widget_key_key] = widget_key
    ta.text_area(
        f"System Prompt für: {active_key}",
        key=widget_key,
        value=st.session_state[buf_key],
        height=500,
        help="Eigene Systemmessage, überschreibt die YAML-Vorlage."
    )